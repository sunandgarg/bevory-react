...
import { PRODUCT_BATCH_CONTENT as BATCH_52 } from "./productContentBatch52.js";
...
describe("researched product batch 52", () => {
  it("contains 30 complete, previously uncovered editorial profiles", () => {
    const earlier = Object.entries(import.meta.glob<{ PRODUCT_BATCH_CONTENT: Record<string, unknown> }>("./productContentBatch[0-5][0-9].ts", { eager: true }))
      .filter(([path]) => !path.endsWith("productContentBatch52.ts"))
      .map(([, batch]) => batch);
    const entries = Object.entries(BATCH_52);
    ...
    for (const [slug, item] of entries) {
      ...
      expect(productContentBatchVersion(slug), slug).toBe("researched-product-batch-52");
