...
describe("researched product batch 15", () => {
  it("contains 30 complete, distinct records", () => {
    const entries = Object.entries(BATCH_15);
  ...
  it("adds editorial without changing catalogue identity or price", () => {
    const original = { slug: "cloudy-bay-pinot-noir-2473a69", id: "id", price: 123, description: "generic" };
    const result = applyProductContentOverlay(original);
    expect(result.id).toBe("id");
    expect(result.price).toBe(123);
    expect(result.product_content_version).toBe("researched-product-batch-15");
    expect(result.description).toContain("Cloudy Bay Pinot Noir");
