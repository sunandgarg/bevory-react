...
describe("researched product batch 16", () => {
  it("contains 30 distinct, complete records with bounded metadata", () => {
    const entries = Object.entries(BATCH_16);
    expect(entries).toHaveLength(30);
    for (const [slug, item] of entries) {
      expect(slug).toMatch(/^[a-z0-9-]+$/);
      expect(ALL[slug], slug).toBe(item);
      expect(productContentBatchVersion(slug), slug).toBe("researched-product-batch-16");
  ...
  it("adds editorial without changing product identity or price", () => {
    const original = { slug: "sensi-chianti-collezione-docg-511f191", id: "id", price: 123, description: "generic" };
    const result = applyProductContentOverlay(original);
    expect(result.id).toBe("id");
    expect(result.price).toBe(123);
    expect(result.product_content_version).toBe("researched-product-batch-16");
    expect(result.description).toContain("Sangiovese-led Chianti");
