...
describe("researched product batch 46", () => {
  it("contains 30 complete new catalogue records with bounded metadata", () => {
    const earlier = Object.values(import.meta.glob<{ PRODUCT_BATCH_CONTENT: Record<string, unknown> }>("./productContentBatch[0-4][0-9].ts", { eager: true }))
      .filter((batch) => batch.PRODUCT_BATCH_CONTENT !== BATCH_46);
    const entries = Object.entries(BATCH_46);
    ...
    for (const [slug, item] of entries) {
      ...
      expect(productContentBatchVersion(slug), slug).toBe("researched-product-batch-46");
      ...
      expect(item.foodPairings, slug).toHaveLength(4);
