...
describe("researched product batch 14", () => {
  it("contains 30 complete, distinct records", () => {
    const entries = Object.entries(BATCH_14);
  ...
  it("adds editorial without changing catalogue identity or price", () => {
    const original = { slug: "macallan-the-macallan-18-years-sherry-casks-4b31bc7", id: "id", price: 123, description: "generic" };
    const result = applyProductContentOverlay(original);
    expect(result.id).toBe("id");
    expect(result.price).toBe(123);
    expect(result.product_content_version).toBe("researched-product-batch-14");
    expect(result.description).toContain("Macallan Sherry Oak 18");
