...
describe("researched product batch 49", () => {
  it("contains 30 complete, previously uncovered editorial profiles", () => {
    const earlier = Object.values(import.meta.glob<{ PRODUCT_BATCH_CONTENT: Record<string, unknown> }>("./productContentBatch[0-4][0-9].ts", { eager: true }))
      .filter((batch) => batch.PRODUCT_BATCH_CONTENT !== BATCH_49);
    const entries = Object.entries(BATCH_49);
    ...
    for (const [slug, item] of entries) {
      ...
      expect(productContentBatchVersion(slug), slug).toBe("researched-product-batch-49");
