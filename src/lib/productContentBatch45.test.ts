...
describe("researched product batch 45", () => {
  it("contains 30 complete new catalogue records with bounded metadata", () => {
    const earlier = Object.entries(import.meta.glob<{ PRODUCT_BATCH_CONTENT: Record<string, unknown> }>("./productContentBatch[0-4][0-9].ts", { eager: true }))
      .filter(([path]) => !path.endsWith("45.ts"))
      .map(([, batch]) => batch);
    const entries = Object.entries(BATCH_45);
    ...
    for (const [slug, item] of entries) {
      ...
      expect(productContentBatchVersion(slug), slug).toBe("researched-product-batch-45");
