...
describe("researched product batch 10", () => {
  it("contains 30 new, complete records", () => {
    const previous = { ...BATCH_01, ...BATCH_02, ...BATCH_03, ...BATCH_04, ...BATCH_05, ...BATCH_06, ...BATCH_07, ...BATCH_08, ...BATCH_09 };
    const entries = Object.entries(BATCH_10);
  ...
  it("overlays editorial without changing price or identity", () => {
    const original = { slug: "glenmorangie-the-lasanta-836ead3", id: "id", price: 123, description: "generic" };
    const result = applyProductContentOverlay(original);
    expect(result.id).toBe("id");
    expect(result.price).toBe(123);
    expect(result.product_content_version).toBe("researched-product-batch-10");
    expect(result.description).toContain("The Lasanta");
