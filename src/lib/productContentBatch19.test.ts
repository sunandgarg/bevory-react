...
describe("researched product batch 19", () => {
  it("contains 30 complete, distinct records", () => {
    const entries = Object.entries(BATCH_19);
    expect(entries).toHaveLength(30);
    for (const [slug, item] of entries) {
      expect(slug).toMatch(/^[a-z0-9-]+$/);
      expect(ALL[slug], slug).toBe(item);
      expect(productContentBatchVersion(slug), slug).toBe("researched-product-batch-19");
  ...
  it("adds editorial without changing product identity or price", () => {
    const original = { slug: "batasiolo-barolo-docg-rosso-7a8ec22", id: "id", price: 123, description: "generic" };
    const result = applyProductContentOverlay(original);
    expect(result.id).toBe("id");
    expect(result.price).toBe(123);
    expect(result.product_content_version).toBe("researched-product-batch-19");
    expect(result.description).toContain("Batasiolo Barolo");
