...
describe("researched product batch 25", () => {
  it("contains 30 complete, distinct, publishable records", () => {
    const entries = Object.entries(BATCH_25);
    expect(entries).toHaveLength(30);
    for (const [slug, item] of entries) {
      expect(slug).toMatch(/^[a-z0-9-]+$/);
      expect(ALL[slug], slug).toBe(item);
      expect(productContentBatchVersion(slug), slug).toBe("researched-product-batch-25");
  ...
  it("adds editorial without altering the catalogue price", () => {
    const original = { slug: "big-banyan-shiraz-b8269e0", id: "id", price: 123, description: "generic" };
    const result = applyProductContentOverlay(original);
    expect(result.id).toBe("id");
    expect(result.price).toBe(123);
    expect(result.product_content_version).toBe("researched-product-batch-25");
    expect(result.description).toContain("Big Banyan Shiraz");
