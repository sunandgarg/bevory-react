...
describe("researched product batch 23", () => {
  it("contains 30 complete, distinct records", () => {
    const entries = Object.entries(BATCH_23);
    expect(entries).toHaveLength(30);
    for (const [slug, item] of entries) {
      expect(slug).toMatch(/^[a-z0-9-]+$/);
      expect(ALL[slug], slug).toBe(item);
      expect(productContentBatchVersion(slug), slug).toBe("researched-product-batch-23");
  ...
  it("adds editorial without changing product identity or price", () => {
    const original = { slug: "jura-aged-12-years-single-malt-scotch-whisky-1ada0e7", id: "id", price: 123, description: "generic" };
    const result = applyProductContentOverlay(original);
    expect(result.id).toBe("id");
    expect(result.price).toBe(123);
    expect(result.product_content_version).toBe("researched-product-batch-23");
    expect(result.description).toContain("Jura 12");
