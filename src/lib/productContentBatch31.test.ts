...
describe("researched product batch 31", () => {
  it("contains 30 complete, distinct editorial records", () => {
    ...
    for (const [slug, item] of entries) {
      ...
      expect(item.metaTitle, slug).toBe(`${item.productName} Price, Taste & Review | BevOry`.length <= 60
