...
describe("researched product batch 06", () => {
  it("contains 30 new, complete records", () => {
    const entries = Object.entries(BATCH_06);
    expect(entries).toHaveLength(30);
    for (const [slug, item] of entries) {
      expect(slug).toMatch(/^[a-z0-9-]+$/);
      expect(slug in BATCH_01 || slug in BATCH_02 || slug in BATCH_03 || slug in BATCH_04 || slug in BATCH_05, slug).toBe(false);
  ...
  it("overlays editorial without changing price or identity", () => {
    const original = { slug: "sula-zinfandel-c75211e", id: "id", price: 123, description: "generic" };
    const result = applyProductContentOverlay(original);
    expect(result.id).toBe("id");
    expect(result.price).toBe(123);
    expect(result.product_content_version).toBe("researched-product-batch-06");
    expect(result.description).toContain("Sula Zinfandel");
