...
describe("researched product batch 12", () => {
  it("contains 30 distinct complete catalogue records", () => {
    const entries = Object.entries(BATCH_12);
  ...
  it("overlays editorial while preserving price and identity", () => {
    const original = { slug: "aberlour-12-year-9bfdec0", id: "id", price: 123, description: "generic" };
    const result = applyProductContentOverlay(original);
    expect(result.id).toBe("id");
    expect(result.price).toBe(123);
    expect(result.product_content_version).toBe("researched-product-batch-12");
    expect(result.description).toContain("Aberlour 12");
