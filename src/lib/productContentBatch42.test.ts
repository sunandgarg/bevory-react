...
describe("researched product batch 42", () => {
  it("contains 30 complete catalogue records absent from earlier batches", () => {
    const earlier = Object.entries(import.meta.glob<{ PRODUCT_BATCH_CONTENT: Record<string, unknown> }>("./productContentBatch[0-4][0-9].ts", { eager: true }))
      .filter(([path]) => !path.endsWith("42.ts"))
      .map(([, batch]) => batch);
    const entries = Object.entries(BATCH_42);
    ...
    for (const [slug, item] of entries) {
      ...
      expect(productContentBatchVersion(slug), slug).toBe("researched-product-batch-42");
