...
import { BRAND_CONTENT_BATCH_57, BRAND_IDENTITIES_BATCH_57, BRAND_LOGOS_BATCH_57, BRAND_SOURCES_BATCH_57 } from "../src/lib/brandContentBatch57.js";
...
describe("brand batch 57", () => {
  it("refreshes 30 complete guides and uses only existing logo mappings", () => {
    expect(BRAND_IDENTITIES_BATCH_57).toHaveLength(30);
    const slugs = BRAND_IDENTITIES_BATCH_57.map(([slug]) => slug).sort();
    expect(Object.keys(BRAND_CONTENT_BATCH_57).sort()).toEqual(slugs);
    expect(Object.keys(BRAND_SOURCES_BATCH_57).sort()).toEqual(slugs);
    expect(Object.keys(BRAND_LOGOS_BATCH_57)).toHaveLength(11);

    for (const [slug] of BRAND_IDENTITIES_BATCH_57) {
      const definition = BRAND_EXPANSION.find((item) => item.slug === slug);
      expect(definition, slug).toBeDefined();
      const content = BRAND_CONTENT_BATCH_57[slug];
      const row = buildBrandExpansionData(definition!);
      expect(row.content_version).toBe("brand-public-ui-v3-batch-57");
      expect(auditPublicBrandFields(row), slug).toEqual([]);
      expect(BRAND_SOURCES_BATCH_57[slug].length).toBeGreaterThan(0);
      ...
      expect(row.logo_url).toBe(BRAND_LOGOS_BATCH_57[slug] ?? null);
